...
## Two gaps, both 404 by design

`/dashboard` and `/settings` are called by this app and served by nothing.
`/dashboard` is the near miss: assessment-service serves `/dashboards`, plural,
and a different shape. Both sit in `UNROUTED_PREFIXES`, which makes the dev
server refuse them with a 404 exactly as the gateway does, instead of Vite's SPA
fallback answering `index.html` at HTTP 200 and leaving `res.data.data`
